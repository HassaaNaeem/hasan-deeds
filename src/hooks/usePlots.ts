...
export function usePlots(filters?: {
  status?: string;
  location?: string;
  area?: string;
}) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await api.get<PlotsResponse>(
...
export function useMyPlots() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const response =
        await api.get<ApiResponse<{ plot: Plot; details: PlotDetails }[]>>(
          "/plots/my-plots",
        );
      return response.data;
    },
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const response = await api.get<
        ApiResponse<{ plot: Plot; plotDetails: PlotDetails }>
      >(`/plots/${plotId}`);
      return response.data;
    },
    enabled: !!plotId,
...
export function useApplyPlot() {
  ...
  return useMutation({
    mutationFn: async (plotId: string) => {
      const response = await api.post<PlotStartApplicationResponse>(
        `/plots/${plotId}/apply`,
      );
      return response.data;
    },
